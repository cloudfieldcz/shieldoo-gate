...
pinned to **Trivy v0.71.0** with a per-platform SHA-256 list; an unrecognised
...
| `SHDG_CACHE_DIR` | `~/.cache/shdg` | Override the Trivy binary cache directory. The pinned binary lives at `<cache-dir>/trivy-0.71.0/trivy`. |
