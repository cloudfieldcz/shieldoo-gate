...
- **API-key management** requires the `keys:manage` scope (OIDC operator sessions have it); a key cannot be minted with scopes the caller does not hold. OIDC operator sessions hold the explicit `auth.operatorScopes` set — every defined scope (`admin:read`, `admin:write`, `keys:manage`, `proxy:fetch`, `scan:upload`) — so an interactive admin can mint a key of any scope from the UI, including the default `proxy:fetch` key. (The set is an explicit literal, not `*`, so a newly added scope is not silently granted to operators.)
...
| `SHDG_CACHE_DIR` | `~/.cache/shdg` | Where the bundled Trivy v0.71.1 binary is cached after first download. The binary lives at `<dir>/trivy-0.71.1/trivy`. |
