...
| **Go**     | `.zip` module      | unzip (in gomod adapter)  | `<module>@<version>/LICENSE*` / `COPYING*` via [`google/licensecheck`](https://pkg.go.dev/github.com/google/licensecheck) | ❌ (Trivy does not support Go) | ✅ | Trivy's `SupportedEcosystems` excludes Go, so the gomod adapter runs its own licensecheck-based classifier over LICENSE-family files in the module root. Matches with ≥75% coverage are normalized to SPDX IDs and persisted via `TriggerAsyncLicenseWrite` (generator `gomod-licensecheck`). No vulnerability scan today — Go module security relies on `sum.golang.org` client-side. |
