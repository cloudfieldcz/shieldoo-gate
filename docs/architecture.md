...
| **`shdg` CLI** | `cmd/shdg/` | Cross-platform CI helper: bundles SHA-256-pinned Trivy v0.71.1, auto-detects ecosystem, generates CycloneDX SBOM, uploads to `POST /api/v1/projects/:label/components/:name/scans`, polls scan-run status, exits non-zero on `--fail-on critical|high`. See [docs/cli/shdg.md](cli/shdg.md). |
| **Auth** | `internal/auth/` | OIDC admin authentication (Authorization Code + PKCE), proxy API key authentication (per-user PAT + global token), scope enforcement (proxy requires `proxy:fetch`; scan-upload requires `scan:upload`; admin API derives the scope from the HTTP method — `GET`/`HEAD`/`OPTIONS`→`admin:read`, mutations→`admin:write`, with `admin:write` implying `admin:read`; the global token holds `*` (satisfies every check), while OIDC operator sessions are injected the explicit `auth.operatorScopes` set — every defined scope — so an interactive admin passes all checks and can mint an API key of any scope), rate limiter, log redactor |
