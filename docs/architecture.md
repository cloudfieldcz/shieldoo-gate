...
| **Auth** | `internal/auth/` | OIDC admin authentication (Authorization Code + PKCE), proxy API key authentication (per-user PAT + global token), scope enforcement (proxy requires `proxy:fetch`; scan-upload requires `scan:upload`; admin API derives the scope from the HTTP method — `GET`/`HEAD`/`OPTIONS`→`admin:read`, mutations→`admin:write`, with `admin:write` implying `admin:read`; global token `*` and OIDC sessions bypass via injected scopes), rate limiter, log redactor |
