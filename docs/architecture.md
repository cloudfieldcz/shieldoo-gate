...
| **Built-in Scanners** | `internal/scanner/builtin/` | 6 always-on Go-native scanners (hash verifier, install hook analyzer, obfuscation detector, exfil detector, PTH inspector, threat feed checker) plus a typosquat/namespace-confusion scanner enabled by default (`scanners.typosquat.enabled`) |
...
| **Manifest Scanner (vuln-scan)** | `internal/scanner/manifest/` + `internal/component/` | Push-from-CI CycloneDX SBOM ingestion: parses SBOM, fans out to OSV + Trivy in parallel, aggregates findings per `(package, version, CVE-ID)`, persists `components` / `scan_runs` / `scan_findings` / `cve_ignores`. Wired in `cmd/shieldoo-gate/vuln_scan_wiring.go`; see [Vulnerability Scan](vulnerability-scan.md) and [ADR-007](adr/ADR-007-vulnerability-scan.md). |
| **`shdg` CLI** | `cmd/shdg/` | Cross-platform CI helper: bundles SHA-256-pinned Trivy v0.71.2, auto-detects ecosystem, generates CycloneDX SBOM, uploads to `POST /api/v1/projects/:label/components/:name/scans`, polls scan-run status, exits non-zero on `--fail-on critical|high`. See [docs/cli/shdg.md](cli/shdg.md). |
| **Auth** | `internal/auth/` | OIDC admin authentication (Authorization Code + PKCE), proxy API key authentication (per-user PAT + global token), scope enforcement (proxy requires `proxy:fetch`; scan-upload requires `scan:upload`; API-key management requires the dedicated `keys:manage` scope, split out of `admin:write`; admin API derives the scope from the HTTP method — `GET`/`HEAD`/`OPTIONS`→`admin:read`, mutations→`admin:write`, with `admin:write` implying `admin:read`; the global token holds `*` (satisfies every check), while OIDC operator sessions are injected the explicit `auth.operatorScopes` set — every defined scope, including `keys:manage` — so an interactive admin passes all checks and can mint an API key of any scope), rate limiter, log redactor |
...
| **Configuration** | `internal/config/` | YAML config loading with Viper, environment variable overrides (`SGW_` prefix), DB initialization and embedded migrations (SQLite + PostgreSQL, currently 001–039) |
...
4. **Initialize database** — Open SQLite or PostgreSQL, run embedded migrations (currently 001–039 in `sqlite/` or `postgres/` subdirectory), set WAL mode and foreign keys (SQLite)
5. **Initialize cache store** — Create storage backend based on config: local filesystem (default), S3, Azure Blob, or GCS
6. **Register scanners** — Always register the 6 built-in scanners (+ typosquat, enabled by default); conditionally add GuardDog, Trivy, OSV based on config
