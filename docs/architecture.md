...
| **`shdg` CLI** | `cmd/shdg/` | Cross-platform CI helper: bundles SHA-256-pinned Trivy v0.74.0, auto-detects ecosystem, generates CycloneDX SBOM, uploads to `POST /api/v1/projects/:label/components/:name/scans`, polls scan-run status, exits non-zero on `--fail-on critical|high`. See [docs/cli/shdg.md](cli/shdg.md). |
