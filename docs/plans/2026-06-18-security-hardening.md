...
- [x] **T7 — Build provenance / signing.** ✅ Done 2026-06-22. Keyless Sigstore (Fulcio+OIDC+Rekor, no managed key) across the whole release. **Images:** `provenance: mode=max` + `sbom: true` (BuildKit SLSA provenance + CycloneDX SBOM as OCI referrers), `actions/attest-build-provenance` (push-to-registry), `cosign sign` **by digest**. **Binaries:** one `attest-build-provenance` over all 5 `shdg` archives. **Dogfooded SBOMs:** persisted via new `shdg scan --sbom-output` (PR #67), attached (`*.cdx.json`), folded into `SHA256SUMS`, signed with `cosign sign-blob --bundle`. Least-privilege perms (top-level `contents: read`, per-job widening); new actions SHA-pinned (ADR-015). Fail-closed guards on incomplete `SHA256SUMS`/empty SBOM. Security-reviewed (no blockers). [ADR-018](../adr/ADR-018-build-provenance-and-signing.md); docs in [development/ci.md](../development/ci.md). Split as PR #67 (shdg `--sbom-output`) + PR for `release.yml`. **Note:** workflow only runs on a `vX.Y.Z` tag, so the first real exercise is the next release tag. **Follow-up (defense-in-depth):** bind the dogfooded SBOM to the image digest (OCI referrer / in-predicate) — the BuildKit `sbom: true` referrer is already digest-bound.
