...
**Circuit breaker scope:** The per-scanner circuit breaker (5 consecutive failures → 60s cooldown) is consulted **only for `required` scanners**, where an open circuit short-circuits to a fast `overload` error (mapped to 503/quarantine) instead of timing out per attempt. Best-effort scanners are never short-circuited: their verdict is fail-open either way, so skipping them would carry no safety benefit and would only drop the data they produce (SBOM, licenses, vuln findings) for any artifact scanned during the cooldown. Best-effort overload is bounded instead by the bridge concurrency semaphore (`max_concurrent_scans`). **Enrichment-class scanners** (those implementing `scanner.EnrichmentScanner`, e.g. `version-diff`) are also exempt from the breaker regardless of criticality: their failures are artifact-specific (one package's diff is anomalous/oversized/uncacheable) or transient-backend, so they fail closed **per artifact** when required but never open a scanner-wide breaker that fast-fails unrelated, healthy artifacts as `overload`. Such scanners protect their own backend via an internal breaker. See [ADR-013](adr/ADR-013-enrichment-scanner-breaker-exemption.md).
...
**Operation:** Synchronous within the scan pipeline, runs in parallel with all other scanners. Large artifacts (> `max_artifact_size_mb`, default 50 MB) are skipped when first-seen, but an oversized **update** with a cached predecessor returns a `terminal` scanner error so the size limit cannot be used to skip a real diff (see [version-diff operational doc](scanners/version-diff.md#when-does-it-run)). For a `required` scanner a `terminal` error blocks with HTTP 403 rather than 503, because retrying cannot shrink the artifact ([ADR-019](adr/ADR-019-terminal-scanner-errors-block-not-retry.md)). A per-scan timeout (`scanner_timeout`, default 55s) sits under the engine outer cap (`scanners.timeout`, default 60s; invariant validated at startup). Idempotency cache keyed on `(artifact, previous_artifact, ai_model_used, ai_prompt_version)` ensures restarts and re-scans don't burn tokens. Per-package hourly rate limiter (exhausted quota → `throttled`) and an **internal** consecutive-failure breaker (open → `throttled`) protect the LLM backend. version-diff is **enrichment-class**, so it has no engine per-scanner breaker ([ADR-013](adr/ADR-013-enrichment-scanner-breaker-exemption.md)) — transient conditions (predecessor blob missing from cache, SHA mismatch on the previous version, a one-off bridge hiccup) **fail open** (CLEAN, error logged not surfaced), while genuine errors (`UNKNOWN` verdict → `retryable`, oversized update → `terminal`, quota/internal-breaker → `throttled`) are surfaced in `ScanReport.Errored` and fail closed **per artifact** for a `required` scanner without ever opening a scanner-wide breaker that fails unrelated packages. `best-effort` mode degrades all of them to fail-open. `UNKNOWN` verdicts are NOT persisted to preserve cache integrity.
...
**Verdict mapping (GuardDog 3.0).** `scan_local()` returns two detector families in `results`: descriptive `capability-*` detectors (network / process-spawn / filesystem use — which fire on plenty of *legitimate* packages) and real findings (`threat-*` source-code threats plus metadata threats like `typosquatting` / `deceptive_author`), aggregated into a `risk_score` with a `label` (`no_risks_detected` vs `low/medium/high_risk`). The bridge (`classify_guarddog_result` in `scanner-bridge/main.py`) flags **`MALICIOUS`** when a non-capability detector fired **or** GuardDog scored the package as risky, and **`CLEAN`** for capability-only / `no_risks_detected` results. This is deliberate: GuardDog 2.x (semgrep) had no capability family, so the bridge treated any non-empty match as malicious; under 3.0 that rule would false-positive-block every package touching the network or a subprocess, so capability-only noise is now ignored while real threats (including metadata threats that need not move the numeric score) still fail closed.
...
| npm | `package.json` | `license` string/object and deprecated `licenses` array; `UNLICENSED` and unresolvable `SEE LICENSE IN <file>` → `NOASSERTION` (see below) |
...
3. `<licenseUrl>` — `https://licenses.nuget.org/<expr>` URLs are decoded to the SPDX expression in their path (nuget.org generates them mechanically, so this is authoritative); the `https://aka.ms/deprecateLicenseUrl` placeholder is dropped; a small **well-known URL map** (`wellKnownLicenseURL`) resolves stable legacy URLs that carry no SPDX in their path — the `go.microsoft.com/fwlink/?LinkId=329770` MIT redirect, `github.com/dotnet/corefx/blob/*/LICENSE.TXT` (corefx = MIT), `opensource.org/licenses/MIT`, and `apache.org/licenses/LICENSE-2.0` — to their SPDX ID; any other URL is forwarded verbatim and lands in the policy engine's *unknown* bucket (handled by `unknown_action`).

**Bundled-file fallback.** When a nuspec yields no SPDX-resolvable license — only an unresolvable `<licenseUrl>` (e.g. legacy packages like Hangfire.Core 1.8.0 pointing at a GitHub raw URL) or no license element at all — the extractor scans the package root for a conventional license file (`LICENSE.md`, `LICENSE`, `LICENSE.txt`, `LICENCE*`, `COPYING.LESSER`, `COPYING`, in that priority order) and classifies the **first** one that resolves. Priority order matters: packages commonly ship both an LGPL summary in `LICENSE.md` and the verbatim GPL text in `COPYING`, so scanning every file would wrongly surface `GPL-3.0-only`; the first match wins and the scan stops.
...
**Free-text name canonicalization.** After per-ecosystem extraction, each license string is run through the alias map in `internal/sbom/parser.go` (`NameAliasToID`) at store time — this is the single canonicalization point that maps free-text names (e.g. `EUPL v1.2 Licensed` → `EUPL-1.2`, `Expat` → `MIT`, `Modified/New/3-Clause BSD License` → `BSD-3-Clause`) to canonical SPDX IDs. The policy evaluator matches on the **stored, canonicalized** value with a lowercase+trim compare only, so a new free-text spelling that reaches storage unresolved must be added to the alias map (not to the evaluator). Deprecated SPDX IDs are **not** expanded, so a blocklist entry must use the exact canonical ID the alias map emits (the `-only` variants, e.g. `LGPL-3.0-only`, `AGPL-3.0-only`).

**`NOASSERTION` marker.** When an ecosystem declares a proprietary/custom license with no usable SPDX identifier — npm `UNLICENSED`, or a `SEE LICENSE IN <file>` whose bundled text does not classify — the extractor emits the literal `NOASSERTION`. It passes through the alias map and storage unchanged, is not in any policy list by default (so it rides `unknown_action`), and is blocked explicitly with `blocked: ["NOASSERTION"]`. Because the evaluator applies **worst-action-wins** across the license array, `["BSD-3-Clause", "NOASSERTION"]` is blocked even though it also declares a permissive license — the desired strict behavior for packages that grant no license.
...
- **Legacy semgrep scratch (GuardDog ≤2.x):** GuardDog 2.x shelled out to semgrep, whose `semgrep-core` did **not** honour the bridge's `tempfile.tempdir`/`TMPDIR` redirect, so its scratch landed as `semgrep-*` (files **and** dirs) in the shared `/tmp` root rather than under `shieldoo-guarddog/`. **GuardDog 3.0 replaced semgrep with a native source-code engine** and no longer spawns it, so this scratch is no longer produced; the Go janitor keeps the `semgrep-` prefix as a defensive no-op sweep (it also reclaims scratch from a legacy 2.x bridge image mid-rolling-upgrade). The cross-container reclaim works because the **scanner-bridge runs as the same non-root user as the gate** (`sgw`, uid 100/gid 101 — see [`scanner-bridge/Dockerfile`](../scanner-bridge/Dockerfile)): `/tmp` is the shared `bridge-socket` volume mounted **sticky** (`1777`), so under the sticky bit only the file's *owner* may unlink it. Matching uids makes bridge-written scratch `sgw`-owned and therefore reclaimable by the gate's janitor; it also keeps the untrusted-content scanner off root. GuardDog refreshes its bundled top-packages typosquatting lists in place, so that one resources dir (`/opt/venv/lib/python*/site-packages/guarddog/analyzer/metadata/resources`) is `chown`ed to `sgw` in the image.
...
- **Go:** [`internal/scanner/tmpjanitor`](../internal/scanner/tmpjanitor/) runs under the graceful-shutdown context when Trivy is enabled, sweeping `os.TempDir()` for `shieldoo-trivy-*` (dirs), `shieldoo-sbom-*` (files), `shieldoo-gate-*` (**regular files only**), `shieldoo-{azblob,s3,gcs}-cache-*` (**regular files only** — cloud cache download scratch, issue #24), and `semgrep-*` (files **and** dirs — legacy GuardDog ≤2.x semgrep scratch; GuardDog 3.0 no longer produces it, prefix retained defensively, issue #24).
