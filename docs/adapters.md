...
- **Manifest `HEAD` is served from the internal store** for push-allowed names (mirrors `GET`): the docker push/pull client issues a manifest `HEAD` existence probe, so an already-pushed image is answered locally and never proxied upstream. For a push-allowed name that misses the internal store (a fresh push target, or a pull-through the gate cannot authenticate to), an upstream `401`/`403` is mapped to **`404`** — the gate never forwards client credentials upstream, so "cannot verify upstream" means "not present here", and the push client proceeds to `PUT`. Genuine upstream `200`/`404` pass through unchanged. (Without this, the manifest `HEAD` leaked an upstream `401` that aborted `docker push`.)
- **Internal manifests resolve by digest as well as by tag** (`HEAD` and `GET`). `docker pull` resolves a tag and then re-fetches/verifies the manifest by digest; the digest path looks the manifest up via `docker_tags.manifest_digest` (a digest is treated as quarantined if any tag pointing at it is quarantined) and serves the content-addressed body from the durable store. Without by-digest resolution the pull of a pushed image fell through to upstream and failed.
