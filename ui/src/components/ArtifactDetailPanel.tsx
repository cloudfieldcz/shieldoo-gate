  ...
  return (
    <div className="w-96 flex-shrink-0 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="p-4 space-y-4 overflow-y-auto max-h-[calc(100vh-16rem)]">
        ...
        {/* Licenses (from SBOM) */}
        <LicensesSection artifactId={resolvedId!} />
...
function LicensesSection({ artifactId }: { artifactId: string }) {
  const q = useQuery({
    queryKey: ['artifact-licenses', artifactId],
    queryFn: () => artifactLicensesApi.get(artifactId),
    // 404 = no SBOM for this artifact (common for pre-v1.2 rows); don't retry.
    retry: false,
    enabled: !!artifactId,
  })

  // No SBOM row — silently hide the whole section.
  if (q.isError || (q.isSuccess && (!q.data || !q.data.licenses || q.data.licenses.length === 0))) {
    return null
  }
  ...
  return (
    <div className="pt-2 border-t border-gray-100">
      ...
      <div className="flex flex-wrap gap-1.5">
        {data.licenses.map((id) => (
          <span
            key={id}
            className="inline-block px-2 py-0.5 rounded text-xs font-mono bg-blue-50 text-blue-800 border border-blue-200"
            title="SPDX license identifier extracted from the artifact's CycloneDX SBOM"
          >
            {id}
          </span>
        ))}
