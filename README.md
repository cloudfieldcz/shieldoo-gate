...
- **CycloneDX SBOM** — every scanned artifact gets a CycloneDX 1.5 SBOM persisted alongside the cache; the push-from-CI vulnerability-scan ingestion accepts CycloneDX 1.5+ (decodes down to Trivy 0.70's output; the current 0.74.0 pin itself emits 1.7)
- **Push-from-CI vulnerability scanning** — pipelines upload a CycloneDX SBOM (or use the bundled [`shdg`](docs/cli/shdg.md) CLI); the gate runs **OSV-Scanner + Trivy in parallel**, aggregates findings per `(package, version, CVE-ID)`, schedules background rescans (default 6 h) so newly-published CVEs against unchanged code are still caught, and offers a per-component **CVE ignore** lifecycle with audit trail. Optional AI surfaces (3σ anomaly banner, fix-path recommendations, ignore-reason drafter) gated behind `ai_features.enabled`. See [docs/vulnerability-scan.md](docs/vulnerability-scan.md) and [ADR-007](docs/adr/ADR-007-vulnerability-scan.md).
...
| **Version Diff Analysis (AI-driven, v2.0)** | **Cross-version** LLM analysis: compares each new package version against its previously cached predecessor (PyPI, npm, NuGet, Maven, RubyGems). Install hooks and top-level executable code from both sides are diffed, secrets redacted, and the change is judged by an LLM — catching maintainer-takeover and dependency-confusion variants where malicious intent only shows up *between* versions. See [docs/scanners/version-diff.md](docs/scanners/version-diff.md) and [ADR-005](docs/adr/ADR-005-ai-driven-version-diff.md). | Sync |
...
| **AI Scanner** | **Single-version** LLM analysis of install-time scripts. Catches zero-day malicious packages on first sight — no previously cached version needed. Complements Version Diff for first-seen packages and ecosystems where no prior version exists in cache. | Sync |
...
The two LLM-powered scanners are complementary, not redundant: the **AI Scanner** judges a single version on its own (catches a malicious package on first sight, even when no prior version is cached), while **Version Diff Analysis** judges the *change* between a new version and its cached predecessor (catches maintainer-takeover and dependency-confusion attacks where the v1 → v2 diff is the only signal). Together they let Shieldoo Gate flag zero-day malicious packages — obfuscated data exfiltration, novel backdoors, social-engineering lures, and trojanized updates of previously-clean packages — that no static signature has ever seen.
