...
func TestTrivyCachePath_HonoursOverride(t *testing.T) {
	...
	got := trivyCachePath("0.71.0")
	if !strings.HasPrefix(got, "/tmp/shdg-test/trivy-0.71.0") {
		t.Errorf("path %q missing override prefix", got)
	}
}

func TestParseChecksums_FindsAsset(t *testing.T) {
	raw := `aaaa1111  trivy_0.71.0_Linux-64bit.tar.gz
bbbb2222  trivy_0.71.0_macOS-ARM64.tar.gz`
	got, err := parseChecksums(raw, "trivy_0.71.0_macOS-ARM64.tar.gz")
	if err != nil {
		t.Fatal(err)
	}
	if got != "bbbb2222" {
...
func TestParseChecksums_AssetMissing_Errors(t *testing.T) {
	if _, err := parseChecksums("aaaa  other.tar.gz", "trivy_0.71.0_Linux-64bit.tar.gz"); err == nil {
...
func TestExpectedChecksums_AllPinned(t *testing.T) {
	required := []string{
		...
		"trivy_0.71.0_macOS-64bit.tar.gz",
...
func TestEnsureTrivy_ReusesCachedBinary(t *testing.T) {
	...
	cachePath := trivyCachePath("0.71.0")
	...
	defer srv.Close()
	got, err := ensureTrivy("0.71.0", srv.URL)
	if err != nil {
